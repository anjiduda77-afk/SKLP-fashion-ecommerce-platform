...
import { FiArrowLeft, FiPrinter, FiDownload, FiPackage, FiFileText, FiDollarSign } from 'react-icons/fi';
import { toast } from 'react-toastify';
import { sellerService } from '@services/apiServices';
...
export default function OrderLabelPage() {
  ...
  const tabParam = searchParams.get('tab') || 'label';
  ...
  useEffect(() => {
    const fetchReceipt = async () => {
      try {
        setLoading(true);
        const res = await sellerService.getOrderReceipt(orderId);
        if (res.data?.success) {
          setReceipt(res.data.receipt);
        }
      } catch (err) {
        toast.error(err.response?.data?.message || 'Failed to load seller packing document');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownloadPDF = async () => {
    ...
    const filename = `StyleStreet_Seller_Packing_${sanitizedId}.pdf`;
    ...
    try {
      ...
      toast.success(`Packing PDF downloaded: ${filename}`);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-xs uppercase font-bold tracking-widest text-gray-600">
          Loading Store Packing Document...
        </p>
      </div>
    );
  }
  ...
  if (!receipt) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 text-center">
        <p className="text-base font-bold text-gray-800 mb-2">Packing Document Not Found or Unauthorized</p>
        <p className="text-xs text-gray-500 mb-4 max-w-md">
          You only have access to packing labels for orders containing products from your verified store.
        </p>
        <button
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 pb-16 print:bg-white print:p-0">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 shadow-sm print:hidden">
        <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Document Switcher Tabs (Default: Packing Label) */}
          ...
          <div className="flex items-center gap-2">
            {/* Multi-package selector if seller has split packages */}
